...
export function Header({ navigation, settings }: { navigation: NavigationLink[]; settings: SiteSettings }) {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-black/70 backdrop-blur-xl">
    <div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-5 lg:px-10">
      <Link href="/" className="flex items-center" aria-label={logo.mode === "image" ? logo.alt : `${logo.text} home`}>
        {logo.mode === "image" ? (
          <Image src={logo.src} alt={logo.alt} width={180} height={48} priority className="h-8 w-auto max-w-[180px] object-contain" />
        ) : (
          <span className="text-xl font-black uppercase tracking-[.08em] text-white" style={{ color: logo.color }}>
            {logo.text}
          </span>
        )}
      </Link>
